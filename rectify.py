...
def stereo_rectify(filepath, savepath, M1, d1, M2, d2, R, T, img_shape, save=True):    
    ...
    for i, fname in enumerate(img_left):
        ...
        if save:
            if not(os.path.isdir(savepath + '/rectified_left') and os.path.isdir(savepath + '/rectified_right')):
                os.mkdir(savepath + '/rectified_left')
                os.mkdir(savepath + '/rectified_right')
            cv2.imwrite(savepath + '/rectified_left/rectified_left{}.jpg'.format(i), undistorted_l)
            cv2.imwrite(savepath + '/rectified_right/rectified_right{}.jpg'.format(i), undistorted_r)
...
if __name__ == '__main__':
    ...
    stereo_rectify(args['filepath'], args['savepath'], *paras)
